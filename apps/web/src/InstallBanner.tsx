...
export function InstallBanner() {
  ...
  useEffect(() => {
    const installed = () => { remember("installed"); setHidden(true); setEvent(null); track({ event: "pwa_interaction", action: "installed" }); };
    const ready = (e: Event) => { if (!mobile) return; e.preventDefault(); setEvent(e as InstallEvent); track({ event: "pwa_interaction", action: "prompt_shown" }); };
  ...
  return <aside className="install-banner" aria-label="Install iLoveMd">
    <span>{instructions ? manual : "Install iLoveMd on your home screen for easier access."}</span>
    {!instructions && <button disabled={busy} onClick={async () => {
      if (!event) { setInstructions(true); track({ event: "pwa_interaction", action: "instructions_shown" }); return; }
      setBusy(true);
      try {
        await event.prompt();
        const choice = await event.userChoice;
        remember(choice.outcome); setHidden(true); track({ event: "pwa_interaction", action: choice.outcome });
      } catch (error) { setInstructions(true); track({ event: "app_error", feature: "pwa", error_code: errorCode(error) }); }
      finally { setEvent(null); setBusy(false); }
    }}>{event ? "Install" : "How to install"}</button>}
    <button onClick={() => { remember("dismissed"); setHidden(true); track({ event: "pwa_interaction", action: "dismissed" }); }}>Not now</button>
  </aside>;
}
